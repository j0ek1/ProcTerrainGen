...
public class TerrainHeightOffset : TerrainHeightGen // Inherits from terrain height gen script
{
    ...
    public override void Execute(int mapResolution, float[,] heightMap, Vector3 heightmapScale, int[,] biomeMap = null, int biomeIndex = -1, BiomeGen biome = null)
    {
        for (int y = 0; y < mapResolution; y++)
        {
            for (int x = 0; x < mapResolution; x++)
            {
                ...
                float newHeight = heightMap[x, y] + (offset / heightmapScale.y); // Calculate new height
